...
@interface LXCyhCalenbarview ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout>
@property (nonatomic ,strong)UICollectionView * calenCollectionview;
@property (nonatomic , strong)LXCyhCalenbardate * cyhcalendate;
...
@implementation LXCyhCalenbarview

- (id)initD_calenbarframe:(CGRect)frame
{
    if (!_daycalenbarview) {
        _daycalenbarview = [[LXCyhCalenbarview alloc] init];
    }
...
{
    self.cyhcalendate = [[LXCyhCalenbardate alloc] init];
...
- (void)add_Y_M_Dlabel
{
    _Y_M_Dlabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 15, kScreenWidth, 13)];
    _Y_M_Dlabel.font = [UIFont systemFontOfSize:16];
    _Y_M_Dlabel.textAlignment = NSTextAlignmentCenter;
    _Y_M_Dlabel.textColor = [UIColor colorWithHexString:@"#333333"];
//    _Y_M_Dlabel.text = [NSString stringWithFormat:@"%ld-%ld-%ld %@",self.comp.year,self.comp.month,self.comp.day,self.weekstr];
    _Y_M_Dlabel.text = [NSString stringWithFormat:@"%ld-%ld",(long)self.comp.year,(long)self.comp.month];
...
- (void)createweekdayLabel
{
    ...
    for (int i = 0; i < 7; i ++) {
        ...
        self.daycalenlabel = [[UILabel alloc] initWithFrame:CGRectMake(i * self.daycalenbarview.frame.size.width/7, CGRectGetMaxY(_Y_M_Dlabel.frame)+20, self.daycalenbarview.frame.size.width/7, 16)];
        self.daycalenlabel.text = weekarray[i];
        self.daycalenlabel.textAlignment = 1;
        self.daycalenlabel.textColor = [UIColor colorWithHexString:@"#666666"];
        self.daycalenlabel.font = [UIFont systemFontOfSize:16];
...
- (void)creatCollectionview
{
    ...
    self.calenCollectionview = [[UICollectionView alloc] initWithFrame:CGRectMake(0,CGRectGetMaxY(_Y_M_Dlabel.frame), self.daycalenbarview.frame.size.width,self.daycalenbarview.frame.size.height-(15+15)) collectionViewLayout:layout];
    ...
    [self.calenCollectionview registerClass:[LXCalenCollectionViewCell class] forCellWithReuseIdentifier:@"cellID"];
...
{
    LXCyhCalenbardate * daycalen = [[LXCyhCalenbardate alloc] init];
    self.todaycomp = [[NSDateComponents alloc] init];
    ...
    LXCalenCollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cellID" forIndexPath:indexPath];
    if (cell.numberlabel) {
        [cell.numberlabel removeFromSuperview];
        [cell.reservationMarkView removeFromSuperview];
    }
    ...
    if (indexPath.row >(_week_firsday_month - 1 + 7) && indexPath.row < (_days+ (_week_firsday_month + 7))) {
        ...
        [cell TodayLabel:indexPath comp:self.comp todaycomp:_todaycomp week_firsday_month:_week_firsday_month number:self.datesource[indexPath.row -(_week_firsday_month) - 7] todayNumBGcolor:_todayNumBGcolor andHiddeReservationState:NO];
...
//        [cell TodayLabel:indexPath comp:self.comp todaycomp:_todaycomp week_firsday_month:_week_firsday_month  number:self.lastdata[(indexPath.row % 7) -  _l_week_firsday_month]];
        [cell TodayLabel:indexPath comp:self.comp todaycomp:_todaycomp week_firsday_month:_week_firsday_month number:self.lastdata[(indexPath.row % 7) -  _l_week_firsday_month] todayNumBGcolor:_todayNumBGcolor andHiddeReservationState:NO];
...
//        [cell TodayLabel:indexPath comp:self.comp todaycomp:_todaycomp week_firsday_month:_week_firsday_month  number:self.fisrtdata[(self.f_days - self.f_remainder + indexPath.row)]];
        ...
        [cell TodayLabel:indexPath comp:self.comp todaycomp:_todaycomp week_firsday_month:_week_firsday_month number:self.fisrtdata[(self.f_days - self.f_remainder + indexPath.row)] todayNumBGcolor:_todayNumBGcolor andHiddeReservationState:NO];
...
- (void)getdatesource:(NSDate *)date
{
    ...
    for (NSInteger i = 1; i <= _days; i ++) {
        ...
        [_datesource addObject:[NSString stringWithFormat:@"%ld",(long)i]];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row >(_week_firsday_month - 1 + 7) && indexPath.row < (_days+ (_week_firsday_month + 7))){
        ...
        NSString *datestr = [NSString stringWithFormat:@"%ld-%ld",(long)_comp.year,(long)_comp.month];
//        _Y_M_Dlabel.text = [NSString stringWithFormat:@"%ld-%ld-%d %@",(long)_comp.year,(long)_comp.month,(indexPath.row + 1 -_week_firsday_month - 7),week_str];
        _Y_M_Dlabel.text = datestr;
        [self.daydelegate getDatestring:datestr];     //回调一个日期的字符串回去
        LXCalenCollectionViewCell * cell = (LXCalenCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.numberlabel.textColor = [UIColor colorWithHexString:@"#666666"];
